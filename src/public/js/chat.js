_$.ready(function ($) {
        ...
        onMessage: function (e) {
            try {
                ...
                if (data.type === 'message') {
                    var line = $('<article class="uk-comment uk-padding-small uk-position-relative" data-name="' + name + '"><header class="uk-comment-header uk-margin-remove"><h5 class="uk-comment-title">' + data.name + '</h5></header><div class="uk-comment-body"><p>' + data.message + '</p></div></article>')
                    ...
                    chat.find('article[data-name]').get(data.index).remove();
                }
    ...
    chat.find('.uk-button-primary').on('click', function () {
        ...
        if (message.length) {
            $.http.post($hb.uri.root + '/chat-sample/save', {message: {name: name, message: message}}, function () {
                $hb.socket.get('ChatSample').send(message, {name: name, type: 'message'});
            });
    ...
    msgBox.on('keyup', function (e) {
        e.keyCode === 13 && chat.find('.uk-button-primary').trigger('click');
    });
    ...
    chat.on('click', 'a.remove', function (e) {
        ...
        if (name === line.data('name')) {
            $.http.delete($hb.uri.root + '/chat-sample/delete/' + line.index(), function () {
                $hb.socket.get('ChatSample').send('', {name: name, type: 'remove', index: line.index()});
            });
        }
    });
});